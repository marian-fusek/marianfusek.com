    ...
    supabaseClient.channel('wn_state_sync')
      .on('postgres_changes', { event:'*', schema:'public', table:'wn_state', filter:`league_id=eq.${CFG.leagueId}` }, payload => {
        ...
        if (remote && remote.updatedAt > (state.updatedAt || 0)) {
          state = normalizeState(remote); saveLocal(); renderAll(); showToast('League synced');
        }
      }).subscribe();
  } catch (e) {
...
function normalizePlayer(p){
  ...
  return {
    ...
    team: p.team || p.player_id, position: p.position || p.fantasy_positions?.[0] || '',
...
function rosterRow(team, slot){
  ...
  const pts = pointsFor(id), proj = projectedFor(id), locked = isLocked(id) || weekRosterLocked(), status = gameStatusForPlayer(p);
  return `<div class="player-row ${locked?'locked':''}" data-player="${id}">
    <div class="slot">${slotLabel(slot)}</div>
    <img class="avatar" src="${playerPhoto(p)}" alt="" onerror="this.style.visibility='hidden'">
    ...
    <div class="player-points"><strong>${fmt(pts)}</strong><span>${proj?`${fmt(proj)} proj.`:'PPR'}</span></div>
  </div>`;
}

function renderPlayers(){
  const el = $('#view-players');
  let list = players.filter(p => filteredTeams.has(p.team) && (activePosition==='ALL' || p.position===activePosition) && (!query || p.name.toLowerCase().includes(query.toLowerCase())) && playerMatchesFilters(p));
  ...
  el.innerHTML = `
    ...
    <div class="players-toolbar">
      ...
      <select class="sort-select" id="sortSelect"><option value="rank" ${sortMode==='rank'?'selected':''}>Top rated</option><option value="proj" ${sortMode==='proj'?'selected':''}>Projected points</option><option value="name" ${sortMode==='name'?'selected':''}>Name</option></select>
    </div>
  ...
  $('#playerSearch').addEventListener('input', e=>{ query=e.target.value; renderPlayers(); });
...
function setupGuideCard(){
  ...
  return `<section class="setup-card guide-card"><div class="guide-head"><div><div class="eyebrow">Start here</div><h2>Draft-day setup</h2></div><span class="setup-badge ${syncClass}">${syncLabel}</span></div><p class="guide-intro">${syncIntro}</p>${setupInstructions}<ol class="guide-steps"><li><strong>Open the same public URL</strong><span>Both of you use the deployed site, not separate downloaded copies. Browsers enjoy making this sound more complicated than it is.</span></li><li><strong>Name your teams</strong><span>Use the two team cards below, save both names, and add logos only if you feel the league deserves branding.</span></li><li><strong>Draft manually from Players</strong><span>Search a player, open their profile, choose a team, then choose the slot. There is no draft clock or commissioner ceremony—just two adults making increasingly confident decisions.</span></li><li><strong>Set the weekly lineup</strong><span>Use Move player to place starters and bench players. Each week keeps its own lineup, and the bench does not score.</span></li><li><strong>Check Matchup before kickoff</strong><span>Review both rosters, projections, and player game status. A player locks at that player’s kickoff, so late heroics are mostly a scheduling problem.</span></li></ol><p class="guide-note">For the first draft, fill both teams together, then use Matchup as the weekly scoreboard. No login is used by design, so keep the public link between the two of you.</p></section>`;
}
...
function openMoveDialog(id){
  ...
  const options=ALL_SLOTS.filter(slot=>slot!==current && slotEligible(p.position,slot)).map(slot=>{
...
function openAssignDialog(id,teamId){
  ...
  if(!p || !team) return;
  if(weekRosterLocked()){ showToast('This week is complete'); return; }
  const eligible=ALL_SLOTS.filter(slot=>slotEligible(p.position,slot));
