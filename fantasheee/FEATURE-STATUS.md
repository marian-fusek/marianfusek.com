...
| Week-specific scoring across Matchup, Team, Players, League and Feed | DONE | Uses the selected week's stats, so Week 1 totals cannot leak into Week 2. Empty stats responses no longer count as available or replace real ESPN matchup totals with zero. Team totals use starters only. Validated player and D/ST outputs against ESPN week-over-week totals; corrected overlapping 60+ FG counting, multiple return-TD categories, D/ST aliases/position labels/team codes, and yards-allowed tiers. |
...
| Production handoff for `marianfusek.com/fantasheee` | IN PROGRESS · 60% | Updated Supabase function is live. The public site still serves an older app bundle; this local frontend is now `app.js?v=57` and `style-v2.css?v=14`. No static-host upload/deploy workflow is configured in this repository. |
